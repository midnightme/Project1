...
class Abgabe(object):
    ...
    def __init__(self, name = '', basename = '', pad = False, **kwargs):
        ...
        if 'xls_filename' in kwargs:
            if 'sheet' in kwargs:
                load_kwargs = {k: kwargs[k] for k in kwargs if k in ['header', 'index_col', 'format', 'indextype']}
                self.load_xls(filename = kwargs['xls_filename'], sheet_name = kwargs['sheet'], **load_kwargs)
    ...
    def load_xls(self, filename, indextype = 'DateTimeIndex', format = None, sheet_name = 0, header = 0, index_col = 0):
        if format == None:
            format = self.native_freq
        self.df = pd.read_excel(os.path.join(self.basename, filename), index_col = index_col, header = header, sheet_name = sheet_name)
     
        self.indextype = indextype
        if indextype == 'DateTimeIndex':
            self.df.index = pd.to_datetime(self.df.index, format = format)
        elif indextype == 'PeriodIndex':
            self.df.index = pd.PeriodIndex(self.df.index, freq = format)
            
        return

    def load_csv(self, filename, header = 0, index_col = 0):
        self.df = pd.read_csv(os.path.join(self.basename, filename), header = header, index_col = index_col)
        
        return
    
    def resample(self, frequency):
        ''' assume that the values stay constant throughout period'''
        ''' This method does not check whether the last date is the one we actually need. '''
        
        return self.df.resample(frequency, ).ffill()

    ''' I'd like to be able to call the object with (almost) arbitrary period strings and get the right result
        means, I need to check the string format. But this method doesn't properly work'''
    def get_freq_from_string(self, test_string):
        freqlist = ['%Y', '%Y-m', '%m-Y', '%Y-%m-%d', '%d-%m-%Y']
   
        for freq in freqlist:
            if not pd.isnull(pd.to_datetime(test_string, format = freq, errors='coerce')):
                return freq

    def convert_index(self):
        if self.indextype == 'PeriodIndex':
            return self.df.index.to_timestamp()
        else:
            return self.df.index
            
    def get_slice_w_freq(self, dates = [], freq = '1D'):
        ''' Req. method that returns a time series of the levy/tax between two dates in the specified frequency.'''
        if len(dates) == 2:
            first = pd.to_datetime(dates[0])
            last = pd.to_datetime(dates[1])
        else:
            raise ValueError("dates has wrong length")
        
        # work on local copy
        df = self.df.copy()
        df.index = self.convert_index()
        
        if df.index[0] > first:
            raise ValueError('first date before available data')
        # time series is shorter than requested date: insert Nat and pad
       
        if df.index[-1] < last:
            df.loc[last] = pd.NaT
            df.fillna(method = 'ffill', inplace = True)
        
        first_index = df[df.index <= first].index.max()
        last_index = df[df.index >= last].index.min()
      
        return df.loc[first_index:last_index].resample(freq).ffill()

    def pad_merged_cells(self, axis = 1):
        ''' merged cells in Excel don't show up as NaN. Thus fill from left to right'''
        self.df = self.df.fillna(method = 'ffill', axis = axis)
